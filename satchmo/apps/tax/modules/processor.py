...
class BaseProcessor(object):
    ...
    def process(self, order=None):
        ...
        Returns tuple (tax amount, tax overview)
            tax overview is a dictionary with taxClass names as keys and tax subtotals as values or empty dictionary.
            tax_total == sum(tax_overview.keys())
        Example:
            (Decimal('30.00'), {'10%': Decimal('20.00'), 'Shipping': Decimal('10.00')})
            Is used by "Order.force_recalculate_total(..)" and many times in "payment".
        Descendants may rename keys to arbitrary texts.
            super(BaseProcessor, self).process(order)
        """
        raise NotImplementedError()
    ...
    def get_rate(self, taxclass=None, *args, **kwargs):
        """
        Tax / price
        Positional parameters should be added in this order, if implemented:
            taxclass=None, area=None, country=None, get_object=False, **kwargs
        Recommendation: Should be called only with tax_class as positional parameter adn other should be named 
        """
        return self.by_price(taxclass, Decimal('1.00'))
